...
def days(in_date, in_time, out_date,out_time):
	""" return no. of days of stay """
	
	l1 = dt.datetime(2015, 1, date, 10, 0)
	u1 = dt.datetime(2015, 1, ++date, 17, 0)          # Replace date with DAY-ZERO OF SAARANG and subsequently the other date variables
	l2 = dt.datetime(2015, 1, date, 9, 0)
	u2 = dt.datetime(2015, 1, ++date, 17, 0)
	l3 = dt.datetime(2015, 1, date, 9, 0)
	u3 = dt.datetime(2015, 1, ++date, 17, 0)
	l4 = dt.datetime(2015, 1, date, 9, 0)
	u4 = dt.datetime(2015, 1, ++date, 17, 0)
	l5 = dt.datetime(2015, 1, date, 9, 0)
	u5 = dt.datetime(2015, 1, ++date, 10, 0)
...
# Renders the html content with context onto the html template
	template = get_template('files/templates/portals/hospi/saar.html')
...
# Returns PDF as HttpResponse object
	file.seek(0)
	pdf = file.read()
	file.close() # Don't forget to close the file handler
	response = HttpResponse(pdf, mimetype='application/pdf')

# Downloads PDF directly to the computer when the link is clicked
...
# Render html content through html template with context
	template = get_template('files/templates/portals/hospi/checkout_bill.html')
	html = template.render(Context(data))
