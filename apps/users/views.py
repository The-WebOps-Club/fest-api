...
def socialauth_error(request, *args, **kwargs):
    messages.error(request,'<strong>OW, Snap!</strong> There was some problem with your login. Please try again after some time!',extra_tags='alert-error')
...
def socialauth_disconnect(request, backend, association_id=None):
    ...
    soc = user.social_auth.get(provider = backend)
    
    if soc.allowed_to_disconnect(user, backend, association_id):
        soc.delete()
    else:
        # messages.error(request,'<strong>Ouch!</strong> This is the only way you can login ! <br />We cannot disconnect this as you won\'t be able to access Bloodline without it.',extra_tags='alert-error')
        return HttpResponseRedirect(reverse('accounts.views.profile'))
    return HttpResponseRedirect(reverse('accounts.views.disconnected'))

def socialauth_completed_view(request, backend, *args, **kwargs):
    try:
        ...
        msg = u'<strong>OW, Snap!</strong> There seems to have been some problem while processing your request. Please try again. <br /> Here\'s what we know: ' + str(e) 
    messages.error(request, msg, extra_tags='alert-error')
    return HttpResponseRedirect(reverse('home'))

def new_association(request, *args, **kwargs):
    messages.success(request,'<strong>Who\'s awesome?</strong> YOU\'re awesome! Successfully connected!',extra_tags='alert-success')
    return HttpResponseRedirect(reverse('accounts.views.profile'))

def disconnected(request, *args, **kwargs):
