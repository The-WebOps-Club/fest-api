...
class HospiTeam(models.Model):
    ...
    date_of_arrival = models.DateField(blank=True, null=True, default='2016-01-06')
    time_of_arrival = models.TimeField(blank=True,null=True, default='10:00:00')
    date_of_departure =  models.DateField(blank=True, null=True, default='2016-01-11')
    ...
    def get_male_members(self):
        M=['male', 'Male', 'm', 'M']
        mem = list(self.members.filter(gender__in=M))
        if self.leader.gender in M:
    ...
    def get_female_members(self):
        F=['female', 'Female', 'f', 'F']
        mem = list(self.members.filter(gender__in=F))
        if self.leader.gender in F:
            mem.append(self.leader)
        return mem
